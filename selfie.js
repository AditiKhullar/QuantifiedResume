var post=d3.json("AditiJsonProfile.json", function(error, data) {
	...
	var skills=[["Java",0],["Algorithms",0],["SQL",8],["Unix",7],["Data Structures",7],["C",3],["C++",3],["Python",3]];
	var skills2=new Array(skills.length);
	d3.csv("Endorsement Info-2.csv", function(error, data){
		data.forEach(function(d) {
        	d.date = d['Endorsement Date'];
        	d.skill = d['Skill Name'];
    	});
    	for(var i=0;i<skills.length;i++){
    		skills2[i]=new Array(2);
    		skills2[i][0]=skills[i][0];skills2[i][1]=0;
    	}
    	console.log(skills2);
    	var start=0;
    	while(data[start].date.substring(0,4)>year_choose) start++;
    	while(data[start].date.substring(0,4)==year_choose&&data[start].date.substring(5,7)>month_choose) start++;
    	for(var i=start;i<data.length;i++){
    		var j=0;
    		while(j<skills2.length&&data[i].skill!=skills2[j][0]) j++;
    		if(j<skills2.length)
    			skills2[j][1]++;
    	}
	});
